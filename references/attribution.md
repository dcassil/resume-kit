...
## Ported Files — Phase 4 (`packages/policy`, `packages/evidence`, `packages/alignment`) (RIT-I-0005)

> Concrete resume-kit files ported or adapted from upstream during Phase 4 — Controlled Alignment.
> Each source file listed below carries the modified-source marker comment block (upstream path +
> pinned SHA `116f9cc3b00e1ac91734a6c2679bf41ea64a0edc`, Apache-2.0). All modifications invert
> app-level dependencies: no file imports `app.*`, `litellm`, `openai`, or any concrete LLM
> provider directly. Files marked "New" below are original resume-kit code with no upstream
> material to attribute.

| resume-kit file | Upstream source | Ported/adapted material | Modification note |
|---|---|---|---|
| `packages/policy/src/resume_kit_policy/path_policy.py` | `app/services/improver.py` (`:80` `_ALLOWED_PATH_PATTERNS`, `:94` `_BLOCKED_PATH_PREFIXES`, `:101` `_BLOCKED_FIELD_NAMES`, `:118` `_is_path_allowed`, `:123` `_is_path_blocked`) | Freedom-aware allowed/blocked path policy | Extracted path-gate substrate from the improver mega-module; exposed as pure functions; generalized into graduated freedom 0–10 policy layer; `PolicyDecision`/`PolicyReasonCode` schema used in place of raw booleans; no LLM or `app.*` imports |
| `packages/policy/src/resume_kit_policy/sanitizer.py` | `app/services/improver.py` (`:29` `_INJECTION_PATTERNS`, `:48` `_sanitize_user_input`) | Prompt-injection sanitizer | Extracted from the improver mega-module; private helper renamed to public `sanitize_user_input`; injection patterns and `[REDACTED]` substitution preserved exactly; no LLM or `app.*` imports |
| `packages/policy/src/resume_kit_policy/truthfulness.py` | `app/prompts/templates.py` (`:211` `CRITICAL_TRUTHFULNESS_RULES_TEMPLATE`, `:230` `CRITICAL_TRUTHFULNESS_RULES`; no-fabrication clauses) | Truthfulness-rule constants | Ported `CRITICAL_TRUTHFULNESS_RULES_TEMPLATE`, `_build_truthfulness_rules`, and the `{nudge, keywords, full}` mapping verbatim into a provider-free module; rule text preserved exactly; no LLM or `app.*` imports |
| `packages/policy/src/resume_kit_policy/skill_targets.py` | `app/services/improver.py` (`:376` `_build_allowed_skill_target_keys`, `:709` `_extract_jd_skill_index`, `:754` `verify_skill_target_plan`) | Verified skill-target gate | Extracted and adapted private helpers into public policy module; replaced upstream dict return values with `SkillTargetPlan`/`SkillTarget`/`SkillTargetRejection` schemas; evidence-backed support and duplicate-target recording are new; no LLM or `app.*` imports |
| `packages/evidence/src/resume_kit_evidence/predicates.py` | `app/tests/evals/scorers.py` (`:100` `sections_preserved`, `:117` `no_fabricated_employers`, `:138` `personal_info_unchanged`, `:161` `flatten_resume_text` helpers) | Structural truth predicates | Extracted pure structural scorers; replaced `app.schemas.ResumeData` with `resume_kit_schemas.ResumeDocument`; `jd_keywords_present`/`is_valid_resume` deliberately omitted (live in `packages/matching`); all deterministic text/comparison logic faithfully preserved |
| `packages/evidence/src/resume_kit_evidence/fabrication.py` | `app/services/refiner.py` (`:290` `validate_master_alignment`, `:591` `fix_alignment_violations`) | Fabrication detection and removal | Extracted deterministic truth guard; replaced `app.schemas.*` with `resume_kit_schemas.analysis` (`AlignmentReport`/`AlignmentViolation`); logging removed; all deterministic re/text/comparison logic faithfully preserved |
| `packages/evidence/src/resume_kit_evidence/builder.py` | — (New) | Candidate-evidence extraction from resume | Original resume-kit code; no upstream material ported |
| `packages/evidence/src/resume_kit_evidence/truth.py` | — (New) | Resume truth-validation and provenance classification | Original resume-kit code; composes `predicates` and `fabrication` modules from this package |
| `packages/alignment/src/resume_kit_alignment/apply.py` | `app/services/improver.py` (`:226` `apply_diffs`, `:215` `_verify_original_matches`, `:289–408` reject branches, `:335–363` reorder salvage) | Diff application engine | Extracted from the improver mega-module; path allow/block gates and skill-target gates delegated to `resume_kit_policy`; `ChangeProposal`/`ChangeSet`/`PolicyRejection` schemas replace upstream `ResumeChange`; deep-copies caller input; no LLM or `app.*` imports |
| `packages/alignment/src/resume_kit_alignment/diff.py` | `app/services/improver.py` (`:42–45` `DiffConfidence`, `:1224–1457` `calculate_resume_diff` and helpers) | Structured diff computation | Extracted deterministic field-level diff; retargeted `app.schemas.ResumeFieldDiff`/`ResumeDiffSummary` to `resume_kit_schemas.change.Diff`/`ResumeDiffSummary`; order-ignored set comparisons emit deterministic source-order results; logging and app dependencies removed |
| `packages/alignment/src/resume_kit_alignment/verify.py` | `app/services/improver.py` (`:115` `_METRIC_RE`, `:430–445` `_count_description_words`, `:448–504` `verify_diff_result`) | Structural diff verifier | Extracted local verifier; upstream bare string warnings retargeted to `resume_kit_schemas.common.Warning`; unused app/LLM dependencies removed; checks remain deterministic |
| `packages/alignment/src/resume_kit_alignment/generation.py` | `app/services/improver.py` (`:506` `generate_resume_diffs`, `:839` `generate_skill_target_plan`, `:604` `_prepare_keywords_for_prompt`, `:754` `_prepare_skill_targets_for_prompt`) | Provider-injected proposal generation | Adapted behind injected `StructuredCompletionProvider` boundary; direct `app.llm`/`app.prompts` coupling removed; parses proposals only — never returns a final applied resume; skill targets routed through deterministic `verify_skill_target_plan` gate |
| `packages/alignment/src/resume_kit_alignment/prompts.py` | `app/prompts/templates.py` (`get_language_name`, `CRITICAL_TRUTHFULNESS_RULES_TEMPLATE`, `CRITICAL_TRUTHFULNESS_RULES`, `DIFF_STRATEGY_INSTRUCTIONS`, `SKILL_TARGET_PLAN_PROMPT`, `DIFF_IMPROVE_PROMPT`) | Prompt templates for proposal generation | Kept only provider-agnostic prompt constants needed by Phase 4 proposal generation; diff prompt explicitly asks for proposals only and forbids final/applied resume output; long prompt lines packaged via implicit string concatenation to satisfy line-length limit without altering emitted text; no `app.*` imports |
| `packages/alignment/src/resume_kit_alignment/review.py` | — (New) | Human-in-the-loop review controller | Original resume-kit code; deterministic state machine, no upstream port |
| `packages/alignment/src/resume_kit_alignment/orchestrator.py` | — (New) | Controlled-alignment orchestration engine | Original resume-kit code; wires all Phase 4 primitives; no line-for-line upstream port |
