"""Import-boundary enforcement for packages/policy.

Scans every .py source file under resume_kit_policy and asserts that
...
def test_no_forbidden_imports() -> None:
    ...
    assert not violations, (
        "Import boundary violations detected in packages/policy:\n"
        + "\n".join(f"  {v}" for v in violations)
    )


def test_source_files_found() -> None:
    """Sanity check: the package source root contains the known policy modules."""
    expected = {
        "path_policy.py",
        "sanitizer.py",
        "skill_targets.py",
