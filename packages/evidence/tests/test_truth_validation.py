...
def test_user_confirmed_from_strong_confirmed_overlap() -> None:
    resume = _resume(
        workExperience=[
            {"company": "Acme", "description": ["Led a team of four engineers daily"]}
        ]
    )
    ...
    assert (
        ...
        is ProvenanceStatus.USER_CONFIRMED
    )
...
def test_unsupported_when_no_overlap() -> None:
    resume = _resume(
        workExperience=[
            {"company": "Acme", "description": ["Orchestrated kubernetes clusters"]}
        ]
    )
    ...
    assert (
        _status_for(report, "workExperience[0].description[0]")
        is ProvenanceStatus.UNSUPPORTED
    )
    assert report.has_unsupported_or_contradicted is True
    assert report.passed is False


def test_contradicted_from_fabricated_skill() -> None:
    resume = _resume(additional={"technicalSkills": ["Haskell"]})
    # Evidence knows only Python; Haskell is fabricated.
    report = validate_resume_truth(resume, _evidence(_skill("Python")))
    assert (
        _status_for(report, "additional.technicalSkills[0]")
        is ProvenanceStatus.CONTRADICTED
    )
    assert report.has_unsupported_or_contradicted is True


def test_contradicted_from_fabricated_certification() -> None:
    resume = _resume(
        additional={
            "technicalSkills": [],
    ...
    report = validate_resume_truth(resume, _evidence(_skill("Python")))
    assert (
        _status_for(report, "additional.certificationsTraining[0]")
        is ProvenanceStatus.CONTRADICTED
    )
