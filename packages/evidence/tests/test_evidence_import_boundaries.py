"""Import-boundary enforcement for packages/evidence.

Scans every .py source file under resume_kit_evidence and asserts that
...
def test_no_forbidden_imports() -> None:
    ...
    assert not violations, (
        "Import boundary violations detected in packages/evidence:\n"
        + "\n".join(f"  {v}" for v in violations)
    )


def test_source_files_found() -> None:
    """Sanity check: the package source root contains the known evidence modules."""
    expected = {
        "builder.py",
        "fabrication.py",
        "predicates.py",
