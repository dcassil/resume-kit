"""Import-boundary enforcement for packages/job-parser.

Scans every .py source file under resume_kit_job_parser and asserts that
...
def test_no_forbidden_imports() -> None:
    ...
    assert not violations, (
        "Import boundary violations detected in packages/job-parser:\n"
        + "\n".join(f"  {v}" for v in violations)
    )


def test_source_files_found() -> None:
    """Sanity check: the package source root contains the known modules."""
    expected = {
        "parse.py",
        "prompts.py",
    }
